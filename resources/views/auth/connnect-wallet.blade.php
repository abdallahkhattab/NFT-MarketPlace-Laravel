...
@push('scripts')
    <script>
        document.getElementById('metamask-btn').addEventListener('click', connectWallet);
        document.getElementById('walletconnect-btn').addEventListener('click', connectWallet);
        document.getElementById('coinbase-btn').addEventListener('click', connectWallet);

        async function connectWallet(event) {
            const walletType = event.target.id.replace('-btn', ''); // Extracts the wallet type
            let walletAddress;

            if (walletType === 'metamask') {
                // MetaMask connection logic (example)
                if (typeof window.ethereum !== 'undefined') {
                    try {
                        ...
                        connectToBackend(walletAddress, walletType);
                    } catch (err) {
                ...
                // WalletConnect connection logic (example)
                // Initialize WalletConnect here
                // For demonstration, let's assume walletAddress is retrieved
                walletAddress = "example_wallet_address_from_walletconnect";
                connectToBackend(walletAddress, walletType);
            } else if (walletType === 'coinbase') {
                // Coinbase connection logic (example)
                // Similar to WalletConnect or MetaMask
                walletAddress = "example_wallet_address_from_coinbase";
                connectToBackend(walletAddress, walletType);
            }
        }

        async function connectToBackend(walletAddress, walletType) {
            try {
                const response = await fetch('get-nonce', {
                    method: 'POST',
                    headers: {
                ...
                if (data.nonce) {
                    ...
                    // Do something with the nonce, like storing it or using it in the app
                } else {
